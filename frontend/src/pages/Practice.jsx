...
export default function Practice() {
  ...
  useEffect(()=>{
    if(trackId){
      api.getTrackQuestions(trackId,10).then(data=>{ setTrackInfo(data.track); loadQuestions({trackId,limit:10}); }).catch(()=>loadQuestions({limit:10}));
  ...
  const handleKeyDown = e => {
    if((e.metaKey||e.ctrlKey)&&e.key==='Enter'){ e.preventDefault(); if(!isLoading&&answer.trim().length>=20&&!feedback) submitAnswer(); }
  };
              ...
              <div style={{ marginTop:'1rem', display:'flex', flexDirection:'column', gap:'0.875rem' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    <button onClick={()=>setShowTips(t=>!t)} style={{ fontSize:12, color:'var(--accent)', background:'none', border:'none', cursor:'pointer', padding:0 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:4 }}>
                  ...
                  <button onClick={submitAnswer} disabled={isLoading||answer.trim().length<20} className="btn btn-primary" style={{ fontSize:13, padding:'9px 20px' }}>
                    {isLoading ? (
